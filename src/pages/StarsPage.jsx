...
const questions = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'b', emoji: '❄️', label: '말 걸어줄 때까지', description: '기다리기' },
      { id: 'c', emoji: '☀️', label: '눈 마주치면', description: '어색하게 웃기' },
      { id: 'd', emoji: '🌈', label: '술자리 진행자', description: '등판하기' },
    ],
  },
  {
    id: 4,
    ...
    options: [
      { id: 'a', emoji: '🎮', label: '온라인 게임', description: '연승 기록 세울 때' },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'b', emoji: '🗣️', label: '친구한테 전화로', description: '한풀이' },
      { id: 'c', emoji: '🎤', label: '노래 들으면서', description: '산책으로 털어내기' },
...
function StarDetailModal({ star, index, onClose, onDelete, stars, onNavigate, nickname, t, language }) {
  ...
  useEffect(() => {
    if (!star?.image_url) return;
    const handler = (e) => {
      if (e.detail?.url !== star.image_url) return;
      if (canvasRef.current) drawStarOnCanvas(canvasRef.current, star);
      if (answersCanvasRef.current && showAnswers) drawStarOnCanvas(answersCanvasRef.current, star);
    };
    window.addEventListener('star-image-loaded', handler);
    return () => window.removeEventListener('star-image-loaded', handler);
  }, [star, showAnswers]);
